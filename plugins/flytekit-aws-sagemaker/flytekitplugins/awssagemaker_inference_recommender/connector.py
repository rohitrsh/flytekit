...
``EndpointPerformances`` list (Advanced jobs) so downstream Flyte tasks can pick
an instance type / endpoint config without re-querying SageMaker.
...
def _build_outputs(describe_response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    recommendations: List[Dict[str, Any]] = [
        _project_recommendation(rec)
        for rec in (describe_response.get("InferenceRecommendations") or [])
    ]
    endpoint_performances: List[Dict[str, Any]] = [
        _project_endpoint_performance(perf)
        for perf in (describe_response.get("EndpointPerformances") or [])
    ]
...
class SageMakerInferenceRecommenderJobConnector(Boto3ConnectorMixin, AsyncConnectorBase):
    ...
    async def create(
        ...
        try:
            ...
            if error_code == "ValidationException" and "Cannot create already existing" in error_message:
                return SageMakerInferenceRecommenderJobMetadata(
    ...
    async def get(
        self, resource_meta: SageMakerInferenceRecommenderJobMetadata, **kwargs
    ) -> Resource:
