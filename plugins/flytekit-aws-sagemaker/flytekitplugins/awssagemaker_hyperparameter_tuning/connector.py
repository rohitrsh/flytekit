...
def _project_best_training_job(
    best: Dict[str, Any], s3_model_artifacts: Optional[str]
) -> Dict[str, Any]:
...
def _build_outputs(
    describe_response: Dict[str, Any], best_s3_model_artifacts: Optional[str]
) -> Dict[str, Any]:
...
class SageMakerHyperParameterTuningJobConnector(Boto3ConnectorMixin, AsyncConnectorBase):
    ...
    async def create(
        ...
        try:
            ...
            if error_code == "ValidationException" and "Cannot create already existing" in error_message:
                return SageMakerHyperParameterTuningJobMetadata(
                    config=config, region=region, inputs=inputs
                )
            # Quota hit: keep the metadata so get() polls and surfaces the limit upstream.
            if (
    ...
    async def _best_training_job_artifacts(
        ...
        try:
            ...
            # Don't fail the whole HPO over a flaky secondary lookup — surface
            # what we have so users still get the rest of the result dict.
            return None
        return (training_describe.get("ModelArtifacts") or {}).get("S3ModelArtifacts")

    async def get(
        self, resource_meta: SageMakerHyperParameterTuningJobMetadata, **kwargs
    ) -> Resource:
        describe_response, _ = await self._call(
            method="describe_hyper_parameter_tuning_job",
            config={
                "HyperParameterTuningJobName": resource_meta.config.get(
        ...
        if current_state == "Completed":
            s3_model_artifacts = await self._best_training_job_artifacts(
                describe_response, resource_meta
            )
    ...
    async def delete(
        ...
        try:
            ...
            if error_code == "ValidationException" and "non-running" in error_message:
                return
            raise e
