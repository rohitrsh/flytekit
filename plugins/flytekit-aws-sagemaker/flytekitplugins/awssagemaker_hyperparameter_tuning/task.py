...
class SageMakerHyperParameterTuningJobTask(AsyncConnectorExecutorMixin, PythonTask):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param config: The boto3 ``create_hyper_parameter_tuning_job``
            configuration. May contain ``{inputs.X}``, ``{images.X}`` and
            ``{idempotence_token}`` placeholders. The
            ``TrainingJobDefinition.AlgorithmSpecification.TrainingImage`` field
            is the most common place to use ``{images.X}``.
        :param region: The AWS region for the SageMaker client. Can be overridden at
            runtime via an ``inputs.region`` literal.
        :param images: A dict of trial training images, referenced from ``config``
            via ``{images.<key>}``. ``ImageSpec`` values are built at
            registration time.
