        ...
        "config": {
            ...
            "StoppingConditions": {
                ...
                "ModelLatencyThresholds": [
                    {"Percentile": "P95", "ValueInMilliseconds": 100}
                ],
            },
        },
        "region": REGION,
        ...
        original_exception=ClientError(
            error_response={
                "Error": {
                    ...
                    "Message": (
                        "Cannot create already existing inference recommendations "
                        "job rec-74443947857331f7"
                    ),
                }
            },
...
async def test_get_advanced_job_emits_endpoint_performances(mock_call):
    """Advanced jobs benchmark user-supplied endpoint configs - those land in EndpointPerformances."""
    mock_call.return_value = (
        {
            "JobName": "rec-adv",
            "JobArn": JOB_ARN,
    ...
    connector = ConnectorRegistry.get_connector("sagemaker-inference-recommender-job")
    metadata = SageMakerInferenceRecommenderJobMetadata(
        config={"JobName": "rec-adv"}, region=REGION
    )
    resource = await connector.get(metadata)
    result = resource.outputs["result"]
    assert result["JobType"] == "Advanced"
