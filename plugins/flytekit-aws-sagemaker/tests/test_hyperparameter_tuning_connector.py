...
async def test_get_completed_swallows_describe_training_failure(mock_call):
    """If the secondary describe_training_job call fails we still return the rest
    of the result; S3ModelArtifacts is just None."""
    ...
    result = resource.outputs["result"]
    assert result["BestTrainingJob"]["TrainingJobName"] == BEST_TRAINING_JOB_NAME
        ...
        original_exception=ClientError(
            error_response={
                "Error": {
                    ...
                    "Message": (
                        "Cannot create already existing hyper parameter tuning job "
                        "xgb-tune-74443947857331f7"
                    ),
                }
            },
...
async def test_create_resource_limit_returns_metadata(mock_call):
    mock_call.side_effect = CustomException(
        message="An error occurred",
        idempotence_token=idempotence_token,
        original_exception=ClientError(
